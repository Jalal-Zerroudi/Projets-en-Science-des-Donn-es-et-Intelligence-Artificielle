...
class COVID19Dashboard:
    ...
    def _update_predictions_display(self, days_list, predictions, mae, conf_low, conf_high, r2, model_name):
        """Met à jour l'affichage des prédictions avec un format de date unifié."""
        # 1) Convertir chaque élément de days_list en str 'YYYY-MM-DD'
        date_strs = [pd.to_datetime(d).strftime('%Y-%m-%d') for d in days_list]

        # 2) Mettre à jour l'entête du modèle
        ...
        self.model_details_label.config(
            text=f"Précision: {accuracy:.1f}% | {len(self.current_df)} points d'entraînement"
        )
        ...
        # 4) Remplir avec les dates formatées
        for date_str, pred, low, high in zip(date_strs, predictions, conf_low, conf_high):
            values = [
                date_str,
                f"{int(pred):,}",
                f"{int(low):,}",
                f"{int(high):,}",
                f"{max(0, min(100, 100 - (abs(high - low) / pred * 50))):.1f}%"
            ]
            self.pred_tree.insert('', tk.END, values=values)

        # 5) Mettre à jour le DataFrame interne
        ...
        # 6) Générer le graphique en convertissant à nouveau en datetime
        self._create_predictions_chart(
            pd.to_datetime(date_strs), predictions, conf_low, conf_high
        )
    ...
    def _create_predictions_chart(self, dates, predictions, conf_low, conf_high):
        """Crée le graphique des prédictions à partir de dates datetime."""
        # dates est un Index/array de datetime
        fig = Figure(figsize=(12, 6), dpi=100)
        ax = fig.add_subplot(111)

        # tracer historique
        ax.plot(
            self.current_df['date'],
            self.current_df['total_cases'],
            label='Historique',
            marker='o'
        )
        # tracer prédictions
        ax.plot(dates, predictions, label='Prédictions', marker='s')

        # intervalles
        ax.fill_between(dates, conf_low, conf_high, alpha=0.2)

        ax.set_title('Prédictions COVID-19')
        ax.set_xlabel('Date')
        ax.set_ylabel('Cas')
        ax.xaxis.set_major_formatter(DateFormatter('%Y-%m-%d'))
        ax.tick_params(axis='x', rotation=45)
